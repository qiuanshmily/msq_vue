  ...
  <div>
    ...
    <div style="margin-top: 15px">
      ...
      <el-row style="margin-top: 10px">
        <el-button type="primary" @click="dialogFormVisible=true" v-if="hasPerm('employee:edit')">人员新增</el-button>
      </el-row>
      ...
      <el-table :data="tableData" stripe v-loading="loading" style="width: 100%;margin-top: 15px">
        ...
        <el-table-column prop="roles" label="拥有角色" width="200">
        </el-table-column>
      ...
      <el-pagination :page-size="10" :current-page.sync="pageNum" layout="total, prev, pager, next" @current-change="queryEmployee('page')" :total="total">
      </el-pagination>
    </div>
...
import { isEmpty } from '../util/utils' //utils中定义的方法isEmpty
  ...
  methods: {
    ...
    submitEmployee () {
      ...
      this.$refs.ruleForm.validate((valid) => {
        // this.$refs.ruleForm 表示取到前面的ref="ruleForm"
        /* validate	对整个表单进行校验的方法，参数为一个回调函数。
        该回调函数在校验结束后被调用，并传入两个参数：是否校验成功和未通过校验的字段。
    ...
    confirmEmp (type) {
      ...
      /* 使用JSON.stringify()把一个JavaScript对象序列化为一个JSON字符串，然后将它保存在变量json中*/
